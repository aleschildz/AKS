...
La primera propiedad de la proposición \ref{propiedades_anillos} muestra que si en un anillo conmutativo $(R, +, \cdot)$ se cumple que $\0 = \1$, entonces $R$ consiste de un solo elemento. En efecto, si eso se cumple, entonces para todo $a \in R$ se cumplirá que
...
Consideremos el anillo $(\mathbb{Z}_4,+,\cdot)$. Si
queremos resolver la ecuación $$ 2\cdot x\ = \ 2\cdot 2,$$ no podemos
``cancelar'' el $2$ en ambos lados de la ecuación y concluir que $x =
2$. De hecho, si bien $x =2$ es una raíz de la ecuación, también lo es $x=0$. El problema aquí es que $2$ no tiene inverso multiplicativo en este anillo, por lo que no es correcto deducir que $x=2$ a partir de la ecuación $ 2\cdot x\ = \ 2\cdot 2$. En general, el anillo $(\mathbb{Z}_n,+,\cdot)$, con $n \geq 2$ un número compuesto, tendrá el mismo problema: existirán elementos $a\neq 0$ y $b\neq 0$
tales que $a\cdot b = 0$. \hfill$\blacksquare$
\end{example}
...
\begin{example}
El anillo conmutativo $(\mathbb{Z}, +, \cdot)$ es un dominio integral. Por lo tanto, la ecuación $2 \cdot x = 2 \cdot 2$ que mencionamos en el ejemplo \ref{ejemplo_no_dominio_integral} sí tiene como única solución a $x = 2$ en esta estructura. 
\hfill$\blacksquare$
\end{example}
...
\begin{itemize}
\item La propiedad 1 de la proposición \ref{propiedades_anillos} nos dice que $\0 = a \cdot \0$, así que $\0 \in (a)$.
...
\item Sea $b \in (a)$, de modo que existe un $r \in R$ tal que $b = a \cdot r$. Por la propiedad 2 de la proposición \ref{propiedades_anillos} tenemos que $-b = a \cdot (-r)$. Luego $-b \in (a)$, y $(a)$ es cerrado bajo tomar inversos aditivos.
\end{itemize} 
\end{proof}
