...
\begin{eqnarray*}
	 ...
	 &=& b^{rm}.
\end{eqnarray*}
Como $a^{rn}=e$ y $b^{rm} = e$, usando el corolario \ref{observacion orden} deducimos que $m \divi rn$ y $n\divi rm$. Como $m$ y $n$ son coprimos, necesariamente $m \divi r$ y $n\divi r$. Así, tenemos que $mn\divi r$, ya que $\MCD(m,n)=1$. Esto concluye la demostración. 
\end{proof}
...
\begin{proof}
Dejaremos fijo $p$ y haremos inducción en $n = |G|$. 
...
El caso base es $n = p$. Como $p \geq 2$, existe un elemento $a \in G$ con $\ord{G}(a) > 1$ (ya que el único elemento con orden $1$ es el nuetro). Por el teorema de Lagrange, tenemos que $\ord{G}(a)$ divide a $n = p$, y, como $p$ es primo, necesariamente $\ord{G}(a) = p$.

Ahora veamos el paso inductivo. Nuevamente consideramos un elemento $a \in G$ con $\ord{G}(a) = m > 1$. Tenemos dos casos:
\begin{itemize}
